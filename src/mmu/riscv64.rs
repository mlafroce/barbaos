...
impl PageTable {
    ...
    pub fn init(&mut self) {
        // Cantidad de páginas en la que divido mi heap (incluyendo paginas de estado)
        let num_pages = self.heap_size / PAGE_SIZE;
        // Cantidad de páginas que necesito para guardar información de las páginas
        let reserved_pages = num_pages / (PAGE_SIZE + 1);
    ...
    pub fn print_allocations(&self) {
        unsafe {
            ...
            let heap_beg = self.heap_start as *const Page;
...
unsafe impl GlobalAlloc for PageAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        if let Some(page_table) = GLOBAL_PAGE_TABLE.as_ref() {
...
unsafe impl Allocator for PageAllocator {
    fn allocate(&self, layout: Layout) -> Result<NonNull<[u8]>, AllocError> {
        if let Some(page_table) = unsafe { GLOBAL_PAGE_TABLE.as_ref() } {
            let pages_needed = (layout.size() >> PAGE_ORDER) + 1;
...
pub static mut GLOBAL_PAGE_TABLE: Option<PageTable> = None;
