...
const PLIC_PENDING: usize = PLIC_INT_BASE;
...
pub fn map_pages(table: &mut MapTable) {
    ...
    table.map(PLIC_INT_ENABLE, PLIC_INT_ENABLE, EntryBits::ReadWrite.val(), 0);
    table.map(PLIC_THRESHOLD, PLIC_THRESHOLD, EntryBits::ReadWrite.val(), 0);
}
